...
<div class="d-flex justify-content-between align-items-center mb-4" data-aos="fade-down">
    ...
    <a href="{{ route('admin.centros.create') }}" class="btn btn-primary btn-sm rounded-pill font-weight-bold shadow-sm"><i class="fas fa-plus mr-1"></i> Nuevo Centro</a>
</div>

<form method="GET" action="{{ route('admin.centros.index') }}" class="card card-outline card-primary mb-4">
    <div class="card-body py-3">
        <div class="input-group">
            <input type="search" name="search" value="{{ request('search') }}" class="form-control rounded-left" placeholder="Buscar por código, nombre, tipo, región, provincia, distrito, localidad, contacto o estado...">
            <div class="input-group-append">
                <button class="btn btn-primary" type="submit"><i class="fas fa-search mr-1"></i>Buscar</button>
                @if(request('search'))
                    <a href="{{ route('admin.centros.index') }}" class="btn btn-outline-secondary">Limpiar</a>
                @endif
            </div>
        </div>
    </div>
            ...
            <tbody>
                @forelse($centers as $center)
                    <tr>
                        ...
                        <td class="text-right">
                            <a href="{{ route('admin.centros.edit', $center) }}" class="btn btn-sm btn-outline-warning rounded-circle" title="Editar"><i class="fas fa-edit"></i></a>
                            <form action="{{ route('admin.centros.destroy', $center) }}" method="POST" class="d-inline" onsubmit="return confirm('¿Confirma la eliminación permanente de este centro?')">
                                @csrf
                                @method('DELETE')
