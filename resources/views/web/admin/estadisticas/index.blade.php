...
<div class="card card-outline card-primary shadow-sm rounded-lg mb-4" data-aos="fade-up">
    <div class="card-body p-3">
        <form action="{{ route('admin.estadisticas.index') }}" method="GET" class="form-inline justify-content-between">
                ...
                @forelse($stats as $row)
                    <tr>
                        ...
                        <td class="align-middle">
                            @if($row->center_code)
                                <span class="badge badge-info px-2 py-1 mr-1">{{ $row->center_code }}</span>
                                <span class="font-weight-bold text-dark">{{ $row->center_name }}</span>
                            @else
                                <span class="text-muted">Todas las páginas del módulo</span>
                            @endif
                        </td>
