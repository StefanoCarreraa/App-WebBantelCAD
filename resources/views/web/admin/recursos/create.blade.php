...
@php($editando = isset($resource))

@section('title', $editando ? 'Editar Recurso - BANDTEL Admin' : 'Subir Nuevo Recurso - BANDTEL Admin')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4" data-aos="fade-down">
    <div>
        <h1 class="h3 font-weight-bold text-dark mb-1">
            <i class="fas fa-file-upload text-primary mr-2"></i>{{ $editando ? 'Editar Recurso Digital o Enlace' : 'Subir Recurso Digital o Enlace' }}
        </h1>
        <p class="text-muted small mb-0">Agregue guías, manuales PDF o enlaces institucionales para los centros CAD / CAU</p>
    </div>
    <a href="{{ route('admin.recursos.index') }}" class="btn btn-outline-secondary btn-sm rounded-pill font-weight-bold">
        <i class="fas fa-arrow-left mr-1"></i> Volver al listado
    </a>
</div>

<div class="card card-outline card-primary shadow-sm rounded-lg" data-aos="fade-up">
    <form action="{{ $editando ? route('admin.recursos.update', $resource->id) : route('admin.recursos.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        @if($editando)
            @method('PUT')
        @endif
        <div class="card-body p-4">
            ...
            <div class="row">
                <div class="col-md-12 form-group">
                    ...
                    <input type="text" name="title" class="form-control rounded-pill @error('title') is-invalid @enderror" value="{{ old('title', $resource->title ?? '') }}" placeholder="Ej: Manual de Alfabetización Digital 2026..." required>
                    @error('title') 
                ...
                <div class="col-md-6 form-group">
                    ...
                    <select name="region_id" class="form-control custom-select rounded-pill @error('region_id') is-invalid @enderror" required>
                        ...
                        @foreach($regions as $region)
                            <option value="{{ $region->id }}" {{ old('region_id', $resource->region_id ?? '') == $region->id ? 'selected' : '' }}>{{ $region->name }}</option>
                        @endforeach
                    </select>
                    @error('region_id') 
                        <span class="text-danger small d-block mt-1">{{ $message }}</span> 
                ...
                <div class="col-md-6 form-group">
                    ...
                    <select name="file_type" class="form-control custom-select rounded-pill @error('file_type') is-invalid @enderror" required>
                        <option value="PDF" {{ old('file_type', $resource->category ?? 'PDF') == 'PDF' ? 'selected' : '' }}>Documento PDF</option>
                        <option value="DOCX" {{ old('file_type', $resource->category ?? '') == 'DOCX' ? 'selected' : '' }}>Documento Word (.docx)</option>
                        <option value="XLSX" {{ old('file_type', $resource->category ?? '') == 'XLSX' ? 'selected' : '' }}>Hoja de Cálculo (.xlsx)</option>
                        <option value="LINK" {{ old('file_type', $resource->category ?? '') == 'LINK' ? 'selected' : '' }}>Enlace Web Externo</option>
                    </select>
                    @error('file_type') 
                        <span class="text-danger small d-block mt-1">{{ $message }}</span> 
                    @enderror
                ...
                <div class="col-md-6 form-group">
                    ...
                    <input type="url" name="external_url" class="form-control rounded-pill @error('external_url') is-invalid @enderror" value="{{ old('external_url', $resource->external_url ?? '') }}" placeholder="https://ejemplo.gob.pe/recurso">
                    @error('external_url') 
                        <span class="text-danger small d-block mt-1">{{ $message }}</span> 
                    @enderror
                </div>
